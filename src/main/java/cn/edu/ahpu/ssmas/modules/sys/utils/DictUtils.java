...
public class DictUtils {
	...
	public static String getDictLabel(String value, String type, String defaultValue){
		if (StringUtils.isNotBlank(type) && StringUtils.isNotBlank(value)){
			for (Dict dict : getDictList(type)){
				if (type.equals(dict.getCategory().getType()) && value.equals(dict.getValue())){
	...
	public static String getDictValue(String label, String type, String defaultLabel){
		if (StringUtils.isNotBlank(type) && StringUtils.isNotBlank(label)){
			for (Dict dict : getDictList(type)){
				if (type.equals(dict.getCategory().getType()) && label.equals(dict.getLabel())){
	...
	public static List<Dict> getDictList(String type){
		...
		if (dictMap==null){
			...
			for (Dict dict : dictDao.findAllList(new Dict())){
				List<Dict> dictList = dictMap.get(dict.getCategory().getType());
				if (dictList != null){
					dictList.add(dict);
				}else{
					dictMap.put(dict.getCategory().getType(), Lists.newArrayList(dict));
				}
			}
