...
<body>
	...
	<form:form id="searchForm" modelAttribute="dict" action="${ctx}/sys/dict/" method="post" class="breadcrumb form-search">
		...
		<form:select id="category" path="category" class="input-medium">
			<form:options items="${fns:getAllDictCategoryList()}" itemLabel="name" itemValue="id" htmlEscape="false"/>
		</form:select>
		&nbsp;&nbsp;
		<label>描述 ：</label>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="dict">
			<tr>
				...
				<td><a href="javascript:" onclick="$('#category').val('${dict.category.id}');$('#searchForm').submit();return false;">${dict.category.name}</a></td>
				<td>${dict.description}</td>
				<td>${dict.sort}</td>
				<shiro:hasPermission name="sys:dict:edit"><td>
    				<a href="${ctx}/sys/dict/form?id=${dict.id}">修改</a>
					<a href="${ctx}/sys/dict/delete?id=${dict.id}&category=${dict.category.id}" onclick="return confirmx('确认要删除该字典吗？', this.href)">删除</a>
    				<a href="<c:url value='${fns:getAdminPath()}/sys/dict/form?category=${dict.category.id}&sort=${dict.sort+10}'><c:param name='description' value='${dict.description}'/></c:url>">添加键值</a>
